	...
	luabridge::getGlobalNamespace(L)
		.beginNamespace("Engine")
			.beginNamespace("ImGUI")
				...
				.addFunction("SetTooltip", +[](const char* text) { ImGui::SetTooltip("%s", text); })
				.addFunction("BulletText", +[](const char* text) { ImGui::BulletText("%s", text); })
				.addFunction("Text", +[](const char* text) { ImGui::TextUnformatted(text); })
				.addFunction("TextColored", +[](const ImVec4& color, const char* text) { ImGui::TextColored(color, "%s", text); })
				.addFunction("TextDisabled", +[](const char* text) { ImGui::TextDisabled("%s", text); })
